...
import ru.org.opslab.common.xml.internal.IndentingXMLStreamWriterEx;
import ru.org.opslab.common.xml.internal.XMLStreamWriterEx;

/**
 * Реализация XmlWriter на стандартном классе IndentingXmlStreamWriter
 */
public class IndentingXmlWriter implements XmlWriter, XmlStrings {

    protected XMLStreamWriterEx stream;
    ...
    public void writeXml(GraphNode node, OutputStream out, boolean head) throws IOException, XMLStreamException {
        ...
        stream = new IndentingXMLStreamWriterEx(f.createXMLStreamWriter(out));
        if (node == null) {
            return;
        }
        if (head) {
    ...
    private void write(InfoNode src) throws XMLStreamException {
        ...
        if (emptyElement) {
            Log.getLogger().flood("Writing empty tag [" + name + "]");
            stream.writeEmptyElement(name);
        } else {
            Log.getLogger().flood("Writing start tag [" + name + "]");
            stream.writeStartElement(name);
        }

        Log.getLogger().flood("Writing attributes");
        ...
        if (!emptyElement) {
            Log.getLogger().flood("Ending tag");
